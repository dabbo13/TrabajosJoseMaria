...
public class DOMParser {
    ...
    public static void muestraNodo(Node nodo, int nivel, PrintStream ps) {
        ...
        for (int i = 0; i < nivel; i++) ps.print(INDENT_NIVEL);
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            // --- NUEVO: Crear nombre de fichero con fecha y hora ---
            SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss");
            String fechaHora = sdf.format(new Date());
            String nombreSalida = "parsing_dom_" + fechaHora + ".txt";

            // --- NUEVO: Crear PrintStream para escribir al fichero ---
            PrintStream ps = new PrintStream(new File(nombreSalida));

            // --- Mostrar la salida en el fichero ---
            muestraNodo(domDoc, 0, ps);

            ps.close(); // Cerrar el fichero
